---
title: "태그 입력을 통한 지식 검색 필터링"
source: "https://console.miso.gs/learn/knowledge/knowledge-tag-setting"
synced_at: "2026-05-30"
nav_title: "태그 입력을 통한 지식 검색 필터링"
---
...
- h1: 태그 입력을 통한 지식 검색 필터링
  - h2: 지식 등록시 태그 등록
    - h3: 일반 문서 (지식 관리 > 지식 추가하기 > 데이터 업로드 하기)
    - h3: Sharepoint 문서 (지식 관리 > 지식 추가하기 > 데이터 업로드 하기)
  - h2: 지식에 입력된 태그 값을 확인하기
  - h2: 지식 검색시 검색 값을 태그와 매핑하기
    - h3: Agent
    - h3: Workflow
  - h2: 지식 검색시 태그를 통한 검색 형태
  - h2: 지식 검색의 형태
...
아래 예시에서는 태그 키 값이 year, 태그 타입은 TEXT, 태그 값은 2017입니다.
태그 값 확인
지식 검색시 검색 값을 태그와 매핑하기
Agent
입력 변수를 설정 하고, 이를 참조할 지식에서 "검색 규칙 설정"을 클릭하여 입력변수의 값과 메타 태그를 연결합니다.
...
아래 예시의 경우 year가 사용자가 넣은 값(year_from) 을 기준으로 >=여부에 대한 체크와 year가 사용자가 넣은 값(year_end)을 기준으로 <= 여부에 대한 체크를 모두 만족하는 지식을 검색하게 됩니다.
Agent 태그 매핑
Workflow
입력 변수를 설정 하고, 이를 참조할 지식에서 "검색 규칙 설정"을 클릭하여 입력변수의 값과 메타 태그를 연결합니다.
