---
title: "[레벨 2] 데이터를 분석하여 CSV 파일로 출력하기"
source: "https://console.miso.gs/learn/tips/workflow-tutorial/google-sheet/level2"
synced_at: "2026-05-30"
nav_title: "[레벨 2] CSV 파일로 출력하기"
---
...
- h1: [레벨 2] 데이터를 분석하여 CSV 파일로 출력하기
  - h2: Google Sheet 도구 노드
    - h3: STEP 1. range 변경하기
  - h2: LLM 노드
    - h3: STEP 2. LLM 노드 구성하기
      - h4: 예제 프롬프트 구조 살펴보기
  - h2: FileOutput 도구 노드
      - h4: CSV 형식이란?
    - h3: STEP 3. FileOutput 도구 구성하기
  - h2: 종료 노드
    - h3: STEP 4. 종료 노드 구성하기
  - h2: 테스트하기
...
이번 예제에서 프롬프트를 아래와 같은 구조로 작성했습니다. 물론 이 방식이 정답은 아닙니다. 중요한 것은 모델이 어떤 역할을 가지고, 어떤 데이터를 보고, 어떤 결과를 만들어야 하는지를 구조적으로 명확하게 작성하는 것입니다.
...
input_fields: 분석 시 참고해야 할 주요 데이터 컬럼을 설명하는 부분입니다. 모델이 어떤 정보를 중심으로 해석해야 하는지 안내하는 역할을 합니다.
output / output_rules: 모델이 어떤 구조와 형식으로 결과를 출력해야 하는지 정의하는 가장 중요한 부분입니다.

또한 원하는 출력 형태가 있다면 examples를 함께 제공하는 것이 매우 중요합니다. 프롬프트에서 예시를 제공하는 것을 shot이라고 하며, 여러 개의 예시를 제공하는 방식을 few-shot 기법이라고 합니다. 이러한 방식은 모델이 원하는 출력 패턴을 보다 안정적으로 따르도록 도와줍니다.

tip! 위 프롬프트를 복사해서 미소AI에게 입력하고 원하는 수정사항을 말하면 빠른 작업이 가능합니다 :)

LLM 노드를 추가한 후, 시스템 프롬프트 영역에 복사한 프롬프트를 붙여넣습니다.
사용자 프롬프트 영역에 values.get 노드의 출력변수 text 를 할당합니다.
사용자 프롬프트 변수 할당
(권장사항) 이 예제는 항상 일정한 CSV 형식의 출력이 필요하므로 LLM의 온도를 낮게 설정하는 것을 권장합니다. 본 예제에서는 온도를 0.2로 설정했습니다.
온도 설정
FileOutput 도구 노드

LLM이 생성한 결과를 그대로 종료 노드에서 텍스트로 출력할 수도 있지만, 데이터를 정리해서 확인해야 하는 경우에는 .csv 파일로 만들어 엑셀에서 확인하는 것이 더 편리합니다.

이때 사용할 수 있는 도구가 File Output 도구입니다.

File Output 도구는 LLM이 생성한 내용을 파일 형태로 만들어 다운로드할 수 있는 링크를 생성해주는 기능입니다. .txt, .md(마크다운), .html과 같이 텍스트 기반으로 이루어진 파일을 생성할 수 있으며, 생성된 파일은 링크를 통해 바로 다운로드할 수 있습니다.
...
LLM의 우측 [+] 버튼을 클릭 - 도구 - fileoutput 도구를 추가합니다.
FileOutput 도구 추가
FileOutput의 상세 설정에서는 아래 두 가지 항목을 설정할 수 있습니다.
항목	설명
Output text	파일 안에 들어갈 텍스트 내용
Output filename	파일 이름 (확장자 포함하여 작성)
Output text에 LLM의 출력 변수를 할당합니다.
Output filename을 입력합니다. 파일명 뒤에는 .csv 확장자를 반드시 붙여야 합니다. 또한 파일명 뒤에 시스템 기본 변수인 datetime을 함께 붙이면, 동일한 파일명이 반복 생성되는 대신 현재 시간이 뒤에 추가된 새로운 파일명이 생성됩니다.
FileOutput 설정
종료 노드
STEP 4. 종료 노드 구성하기
종료 노드의 출력 변수에는 FileOutput 노드의 text 변수를 설정합니다. 이렇게 설정하면 실행 결과에 파일 다운로드 링크가 자동으로 할당되어, 생성된 CSV 파일을 바로 다운로드할 수 있습니다.
종료 노드 구성
테스트하기
