---
title: "지식 구성하기"
source: "https://console.miso.gs/learn/knowledge/build-knowledge"
...
+ 데이터 추가 버튼을 클릭하여 MISO에서 사용할 데이터를 추가합니다.

데이터 추가 화면

파일 업로드 없이 "빈 데이터로 생성" 버튼을 클릭하면, 업로드 없이 지식 생성이 가능합니다.

빈 지식으로 생성 시, 이름 정의 팝업
...
유사도 검색: 사용자로부터 입력받은 검색내용을 임베딩으로 변환하고, 청크와 유사한 의미를 가진 결과를 생성합니다.

키워드 검색: 문서에 포함된 용어를 기반으로, 사용자 검색 내용과 정확히 일치하거나 비슷한 용어를 검색합니다. 텍스트 일치에 초점을 맞춰 원하는 내용을 빠르게 찾을 수 있습니다.

하이브리드 검색(재랭크): 유사도 검색과 키워드 검색을 결합하여 초기 검색 결과를 도출한 뒤, 재랭크 모델로 결과를 재정렬합니다. 단, 재랭크 모델이 가능한 LLM 을 사용해야합니다.
...
문서 테이블: 임베딩된 문서에 대한 정보를 나타내는 테이블 입니다. 애플리케이션에서 활용하기 위해서는 상태가 사용가능 이어야만 합니다. 활성화 여부를 통해 상태 값을 조정할 수 있습니다.
...
지식 대한 기본 설정값과 임베딩, 검색 설정에 대한 전체적인 변경이 필요한 경우에 사용됩니다.

지식 설정

공유 설정: 지식 공유 여부를 정의 합니다. ( 참고 )

비공개: 지식을 등록한 사용자만 사용가능한 상태입니다.(Default)

전체 공개: 모든 사용자가 지식을 사용할 수 있는 상태 입니다.

모든 워크스페이스 공개: MISO 시스템에 가입된 사용자 모두 지식을 사용할 수 있는 상태 입니다.

현재 워크스페이스 공개: 현재 워크스페이스에 소속된 사용자만 지식을 사용할 수 있는 상태 입니다.

부분 공개: MISO 시스템에 가입 된 특정 사용자만 지식을 사용할 수 있는 상태입니다.
